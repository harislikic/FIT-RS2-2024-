...
namespace AutoTrade.Migrations
{
    ...
    IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'UpdateExpiredHighlights')
    EXEC('
        CREATE PROCEDURE UpdateExpiredHighlights
        AS
        BEGIN
            UPDATE dbo.AutomobileAds
            SET IsHighlighted = 0
            WHERE IsHighlighted = 1 AND HighlightExpiryDate <= GETDATE();
        END
    ');
");
